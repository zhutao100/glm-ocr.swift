        ...
        return Stream.withNewDefaultStream {
            ...
            let sequenceMaxNewTokens: [Int] = {
                if let maxNewTokensBySequence {
                    ...
                    precondition(
                        maxNewTokensBySequence.allSatisfy { $0 <= maxNewTokens },
                        "maxNewTokensBySequence must be <= maxNewTokens")
            ...
            let headDim =
                config.textConfig.headDim ?? (config.textConfig.hiddenSize / config.textConfig.numAttentionHeads)
            ...
            let generatedTokenBuffer =
                MLXArray.zeros([batch, maxNewTokens], dtype: .int32) + eosVector.reshaped(batch, 1)
